...
window.addEventListener('appinstalled',function(){
    ...
    if(typeof showToast==='function')showToast('✅ VirMusic berhasil diinstall!');
});
function installPWA(){
    if(deferredInstallPrompt){
        deferredInstallPrompt.prompt();
        deferredInstallPrompt.userChoice.then(function(choice){
            if(choice.outcome==='accepted'&&typeof showToast==='function')showToast('✅ Menginstall VirMusic...');
            deferredInstallPrompt=null;
            var btn=document.getElementById('pwa-install-btn');
            if(btn)btn.classList.add('hidden');
        });
...
var App={
    init(){
        ...
        <div class="pt-12 px-4 text-center">
            ...
            <h1 class="text-3xl font-black chrome-text mb-1">VirMusic</h1>
            ...
            <div class="glass rounded-2xl p-5 max-w-sm mx-auto space-y-3 text-left mb-6">
                ...
                <div class="flex justify-between"><span class="text-white/70 text-sm">Nama</span><span class="text-white font-medium text-sm">VirMusic</span></div>
            ...
            <div class="glass rounded-2xl p-5 max-w-sm mx-auto space-y-4 text-left mb-6">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-white font-bold text-sm">Vir</span>
                    </div>
                </div>
            ...
            <a href="https://whatsapp.com/channel/0029Vb8IZxaFHWqAJe7pld1N" target="_blank" class="block w-full max-w-sm mx-auto btn-chrome font-bold py-4 rounded-full active:scale-95 transition-all text-center">
    ...
    autoPlayTrack(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='VirMusic',cover=toHDCover('', videoId),artistId='';
            ...
            S.ct={id:videoId,videoId:videoId,title:'Lagu',artist:'VirMusic',cover:toHDCover('', videoId),artistId:'',ytUrl:'https://youtube.com/watch?v='+videoId};
            S.ps='direct';S.pl=[S.ct];S.pi=0;UU();MP.show();resetLyricsUI(videoId);
            setTimeout(function(){FullPlayer.open();loadTrack(S.ct);},400);
        });
    },
    showSharePopup(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='VirMusic',cover=toHDCover('', videoId);
        ...
        }).catch(function(){App.renderPopup(videoId,'Lagu','VirMusic',toHDCover('', videoId));});
    },
