...
function updateOG(title,image){
    var t=document.querySelector('meta[property="og:title"]');if(!t){t=document.createElement('meta');t.setAttribute('property','og:title');document.head.appendChild(t);}t.setAttribute('content',title+' | VirMusic');
...
function updateOG(title, cover, artist) {
    if (title && cover) {
        ...
        var docTitle = fullTitle + ' | VirMusic';
        var description = 'Dengarkan ' + fullTitle + ' di VirMusic';
        ...
        document.title = 'VirMusic';

        setMetaTag('og:title', 'VirMusic', true);
        setMetaTag('og:description', 'VirMusic - Web Music Player', true);
        setMetaTag('og:image', 'https://www.gobox.my.id/file/uZKDQkZ3c5VK.png', true);
        setMetaTag('twitter:title', 'VirMusic', false);
        setMetaTag('twitter:description', 'VirMusic - Web Music Player', false);
    ...
    popup.innerHTML = '<div class="w-full max-w-sm rounded-3xl p-6 border border-white/10 glass-strong text-center" style="animation:slideUp 0.3s ease-out forwards; background: var(--bg-color);">' +
        ...
        '<div id="share-card-preview" class="p-6 rounded-2xl mb-6 flex flex-col items-center gap-4 relative overflow-hidden" ' +
            ...
            '<div class="border-t border-white/5 w-full pt-3 mt-1 flex items-center justify-center gap-1.5">' +
                ...
                '<span class="text-[10px] text-[#6b7280] tracking-wider font-semibold uppercase">VirMusic Web App</span>' +
            '</div>' +
        '</div>' +
...
function triggerNativeShare() {
    ...
    if (navigator.share) {
        navigator.share({
            title: S.ct.title,
            text: '🎵 Dengarkan ' + S.ct.title + ' - ' + S.ct.artist + ' di VirMusic!',
            url: url
        }).catch(function() {});
    } else {
        copyShareLink();
...
function downloadShareCard() {
    ...
    img.onload = function() {
        ...
        ctx.fillText('DIDENGARKAN DI VirMusic', 300, 710);
        
        try {
            var dataUrl = canvas.toDataURL('image/png');
            var a = document.createElement('a');
            a.download = S.ct.title.replace(/[^a-zA-Z0-9]/g, '_') + '_VirMusic.png';
            a.href = dataUrl;
            a.click();
